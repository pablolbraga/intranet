...
$_SQL_USUARIO_INTRANET = "SELECT IDUSUARIO AS ID, NOME FROM SR_USUARIO WHERE STATUS = 'A' ORDER BY NOME";
$_SQL_USUARIO_IW = "SELECT PROF.IDPERSON AS ID, PF.NAME AS NOME, PROF.ACTIVE FROM GLBPROFESSIONAL PROF INNER JOIN GLBPERSON PF ON PF.ID = PROF.IDPERSON WHERE PROF.ACTIVE = 1 ORDER BY PF.NAME";
$_SQL_UGB = "SELECT ID, NOME FROM SR_UGB ORDER BY NOME";
        ...
        <form name="form-pesquisa" id="form-pesquisa" method="POST" action="busca.php?tipo=<?php echo $_REQUEST["tipo"] ?>&campocodigo=<?php echo $_REQUEST["campocodigo"] ?>&campodescricao=<?php echo $_REQUEST["campodescricao"] ?>&title=<?php echo $_REQUEST["title"] ?>">
            ...
            <div class="container-fluid">
                ...
                if (isset($_POST["btnPesquisar"])){
                    ...
                    } else if ($_POST["tipo"] == 13){
                        $sql = $_SQL_USUARIO_INTRANET;
                    } else if ($_POST["tipo"] == 14){
                        $sql = $_SQL_USUARIO_IW;
                    } else if ($_POST["tipo"] == 15){
                        $sql = $_SQL_UGB;
                    }
