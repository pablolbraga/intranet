...
$qry = $ctrOrcamento->buscarPorId($_POST["txtOrcamento"]);
...
if (count($qry) > 0){
    ...
    <html>
        ...
        <body>
            <form name="form1" id="form1" method="POST" action="index.php?pag=6">
                <table style="width: 100%">
                    <tr>
                        ...
                        <td style="width: 15%; text-align: center; font-size: 20;">
                            ...
                            <input type="hidden" name="txtOrcamento" value="<?php echo $_POST["txtOrcamento"] ?>" />
                        </td>
                    </tr>
                </table>
                ...
                <table style="width: 100%;">
                    <tr>
                        <td style="width: 100%; text-align: center;">
                            Paciente: <?php echo $qry[0]["NMPACIENTE"] ?> - Orçamento: <?php echo $_POST["txtOrcamento"] ?> - Nro. Atend: <?php echo $qry[0]["IDADMISSION"] ?>
                        </td>
                    </tr>
                </table>
                ...
                <table style="width: 100%;" border="1">
                    ...
                    for($i = 0; $i < count($qry); $i++){
                        $valorDesconto = 0;
                        if ($qry[$i]["VRDESCONTO"] > 0){
                            $valorDesconto = ($qry[$i]["VRDESCONTO"] / 100); 
                        }

                        $subtotal = $subtotal + ($qry[$i]["VRTOTAL"] - ($qry[$i]["VRTOTAL"] * $valorDesconto));
                        ...
                        if (@$_POST["cbxSelecionado" . $i] == "S"){
                            ...
                            <tr style="font-size: 8;">
                                ...
                                <td style="text-align: right;"><?php echo number_format($qry[$i]["VRTOTAL"] - ($qry[$i]["VRTOTAL"] * ($valorDesconto)), 2, ',', '.') ?></td>
                            ...
                            if (@$_POST["txtSelecionado" . $i] == "S"){ 
                                // Solicitado alteração para que fosse excluídos os que não estiverem marcados
                                $sql_del = "DELETE FROM capbudgetitem WHERE ID = " . $qry[$i]["IDITEMORCAMENTO"];
                                //echo $sql_del . "<br>";
                                $daobasico->executarProcedimento($sql_del);
                            } else {
                                ?>
                                <tr style="font-size: 8;">
                                    <td><?php echo $qry[$i]["NMPRODUTO"] ?></td>
